...
This repository uses the layered Spec Kit Memory workflow. Memory is part of every non-trivial task, not optional documentation.

## Memory Layers

- **Constitution / principles**: Read `.specify/memory/CONSTITUTION.md` first. Store only stable operating principles there. Never store feature-specific notes there.
...
- **Ephemeral run context**: Use the current prompt, diff, terminal output, and temporary notes only. Do not commit them to durable memory.
