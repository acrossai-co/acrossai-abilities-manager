...
= 0.0.16 =
* **BREAKING — every ability slug has been renamed to verb-first form.** All 219 abilities are now `acrossai-abilities-manager/<verb>-<subject>` (e.g. `get-site-title`, `activate-theme`, `list-plugins`) instead of the pre-0.0.16 mixed subject-first / verb-first mess (`site-title-get`, `theme-activate`, `plugin-list`). 163 slugs changed, 56 already-verb-first slugs are unchanged. Every ability's `label` was already verb-first ("Get Site Title", "Activate Theme"), so the slug now reads the same word order as the label. Motivation: alignment with MCP / function-calling naming conventions used by every major LLM tool-use spec, and removal of intra-category inconsistency (e.g. `plugin-list` vs `create-post` both existing side-by-side).
* **Automatic one-shot data migration renames stored slugs in-place.** Runs on plugin activation AND on the next `admin_init` after upgrade (both paths idempotent — a `acrossai_abilities_slug_rename_058_done` option flag no-ops repeats). Wraps all UPDATEs in a single transaction. Rewrites three tables: `wp_acrossai_abilities.ability_slug`, `wp_abilities_access_control.key` (scoped to the `acrossai-abilities` namespace), and defensively `wp_acrossai_mcp_server_abilities.ability_slug` in the sibling MCP-manager plugin when that table exists.
* **External callers must update slug references.** Any custom code, saved MCP client configs, ACL entries created outside this plugin's UI, or scripts calling `/wp-json/wp-abilities/v1/abilities/acrossai-abilities-manager/<slug>/run` need their slug strings changed. No backwards-compatibility aliases are shipped. See the full old→new mapping in the plan under `specs/058-slug-rename-verb-first/`.
...
= 0.0.12 =
* **New — WordPress core rollback ability under the Core category.** `acrossai-abilities-manager/rollback-wp-core` rolls back WordPress core to an earlier offered version via WP core's `Core_Upgrader::upgrade()` — the same class the WordPress dashboard uses for forward updates. Fetches the offer list from the WP.org Core API 1.7 endpoint (`https://api.wordpress.org/core/version-check/1.7/`) via `wp_remote_get()`, picks the requested version, and hands the offer directly to the upgrader. Uses only WordPress functions; no bundled updater code. Requires BOTH `manage_options` AND `update_core`; honours `DISALLOW_FILE_MODS` via `File_Mods_Guard`; multisite-guarded; refuses when the target version is equal to or newer than the currently-installed version (steers callers to `wp-core-update`). The per-locale offer list is cached in a site transient with a day-long TTL. Annotated `destructive=true` — rolling WordPress back is a real production operation and clients should surface it accordingly. Inspired by Andy Fragen's [core-rollback](https://github.com/afragen/core-rollback) plugin (MIT-licensed). See PR [#77](https://github.com/acrossai-co/acrossai-abilities-manager/pull/77).
