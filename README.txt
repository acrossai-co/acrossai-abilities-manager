...
* **Automatic one-shot data migration renames stored slugs in-place.** Runs on plugin activation AND on the next `admin_init` after upgrade (both paths idempotent — a `acrossai_abilities_slug_rename_058_done` option flag no-ops repeats). Wraps all UPDATEs in a single transaction. Two phases: (1) per-slug UPDATE for each of the 163 word-order flips (rewriting both prefix and suffix atomically); (2) bulk `REPLACE(slug, 'acrossai-abilities-manager/', 'acrossai/')` catches the 56 unchanged-suffix rows. Rewrites three tables: `wp_acrossai_abilities.ability_slug`, `wp_abilities_access_control.key` (scoped to the `acrossai-abilities` namespace), and defensively `wp_acrossai_mcp_server_abilities.ability_slug` in the sibling MCP-manager plugin when that table exists.
* **External callers must update slug references.** Any custom code, saved MCP client configs, ACL entries created outside this plugin's UI, or scripts calling `/wp-json/wp-abilities/v1/abilities/acrossai-abilities-manager/<old-slug>/run` need their slug strings changed to `/wp-json/wp-abilities/v1/abilities/acrossai/<new-slug>/run`. No backwards-compatibility aliases are shipped.
