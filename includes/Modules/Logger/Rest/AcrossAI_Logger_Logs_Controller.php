...
class AcrossAI_Logger_Logs_Controller extends WP_REST_Controller {
		...
		register_rest_route(
			...
			array(
				...
				'args'                => array(
					'search'       => array(
						'type'              => 'string',
						'description'       => __( 'Search by ability slug (partial match)', 'acrossai-abilities-manager' ),
						'required'          => false,
						'sanitize_callback' => 'sanitize_text_field',
					),
					'orderby'      => array(
						'type'        => 'string',
						'description' => __( 'Column to sort by', 'acrossai-abilities-manager' ),
						'required'    => false,
						'default'     => 'created_at',
					...
					'ability_slug' => array(
						'type'              => 'string',
						'description'       => __( 'Filter by ability slug (exact match)', 'acrossai-abilities-manager' ),
						'required'          => false,
						'sanitize_callback' => 'sanitize_key',
					),
					'user_id'      => array(
						'type'        => 'integer',
						'description' => __( 'Filter by user ID', 'acrossai-abilities-manager' ),
						'required'    => false,
					),
					'page'         => array(
						'type'              => 'integer',
						'description'       => __( 'Page number', 'acrossai-abilities-manager' ),
						'required'          => false,
						'default'           => 1,
					...
					'per_page'     => array(
						'type'              => 'integer',
						'description'       => __( 'Records per page (max 100)', 'acrossai-abilities-manager' ),
						'required'          => false,
						'default'           => 20,
						'sanitize_callback' => 'absint',
					),
	...
	public function get_item_schema() {
		return array(
			...
			'properties' => array(
				'id'            => array(
					'description' => __( 'Log entry ID', 'acrossai-abilities-manager' ),
					'type'        => 'integer',
					'context'     => array( 'view' ),
				),
				'ability_slug'  => array(
					'description' => __( 'Ability slug', 'acrossai-abilities-manager' ),
					'type'        => 'string',
					'context'     => array( 'view' ),
				),
				'source'        => array(
					'description' => __( 'Execution source (mcp, rest, cli, cron, ajax, direct)', 'acrossai-abilities-manager' ),
					'type'        => 'string',
					'enum'        => array( 'mcp', 'rest', 'cli', 'cron', 'ajax', 'direct' ),
					'context'     => array( 'view' ),
				),
				'mcp_server_id' => array(
					'description' => __( 'MCP server ID (null if not from MCP)', 'acrossai-abilities-manager' ),
					'type'        => array( 'string', 'null' ),
					'context'     => array( 'view' ),
				),
				'user_id'       => array(
					'description' => __( 'User ID (0 for non-user contexts)', 'acrossai-abilities-manager' ),
					'type'        => array( 'integer', 'null' ),
					'context'     => array( 'view' ),
				),
				'input'         => array(
					'description' => __( 'Execution input (JSON or null)', 'acrossai-abilities-manager' ),
					'type'        => array( 'string', 'null' ),
					'context'     => array( 'view' ),
				),
				'output'        => array(
					'description' => __( 'Execution output (JSON or null)', 'acrossai-abilities-manager' ),
					'type'        => array( 'string', 'null' ),
					'context'     => array( 'view' ),
				),
				'status'        => array(
					'description' => __( 'Execution status', 'acrossai-abilities-manager' ),
					'type'        => 'string',
					'enum'        => array( 'success', 'error', 'permission_denied' ),
					'context'     => array( 'view' ),
				),
				'duration_ms'   => array(
					'description' => __( 'Execution duration in milliseconds', 'acrossai-abilities-manager' ),
					'type'        => 'integer',
					'context'     => array( 'view' ),
				),
				'created_at'    => array(
					'description' => __( 'Timestamp in format YYYY-MM-DD HH:MM:SS', 'acrossai-abilities-manager' ),
					'type'        => 'string',
					'format'      => 'date-time',
					'context'     => array( 'view' ),
				),
