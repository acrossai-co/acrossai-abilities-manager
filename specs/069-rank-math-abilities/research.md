...
| The plugin already ships generic raw-option abilities | `options/get-option`, `update-option`, `list-options`, `search-options`, `patch-option-value`, `get-nested-option-value`, `delete-option` | Decisive argument for **not** adding Rank Math-specific raw option abilities: the escape hatch already exists, so the Rank Math surface only offers the typed path |
| `users/add-role-capability` / `remove-role-capability` already grant/revoke any WP capability | `includes/Abilities/Users/` | `rank_math_*` caps are WP caps, so no bulk Rank Math role-cap writer is needed |
| `settings/list-rewrite-rules` already returns the full persisted rewrite map | `includes/Abilities/Settings/List_Rewrite_Rules.php` | `rank-math-get-rewrite-status` dropped as redundant |
