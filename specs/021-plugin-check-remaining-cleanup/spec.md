...
- **FR-010a**: `AcrossAI_Abilities_Sanitizer.php` MUST remove `php_code` from its allowed callback type list. No rename to a new type is performed in this feature; the Sanitizer mirrors the DB-remove-only decision (FR-010c).
...
- **FR-010c**: `AcrossAI_Abilities_Query.php` MUST remove `php_code` from its database enum allowlist so the storage layer rejects the deprecated type. No new `registered_callback` enum type is introduced in this feature; the new type and its DB column value are deferred to the follow-up spec that ships the UI (FR-010e).
...
- Q: Should Feature 021 also add a new `registered_callback` DB enum type in `AcrossAI_Abilities_Query.php`, or only remove `php_code`? → A: Remove `php_code` only. The new callback type and DB column value are deferred to the follow-up spec that ships the UI (FR-010e). FR-010c updated accordingly.
