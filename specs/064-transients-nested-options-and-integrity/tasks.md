...
- [ ] **T030** [US3] Create `includes/Abilities/Content/Add_Post_Meta.php` implementing `content/add-post-meta` per contracts §7. Mirror the input-schema structure of `Update_Post_Meta.php` (accept `key`/`meta_key` and `value`/`meta_value` aliases). Add `unique: bool = false`. `execute()`:
