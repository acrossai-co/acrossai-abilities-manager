...
- [x] T015 [P] Bump `wpboilerplate/wpb-access-control` constraint in `composer.json` from `^1.2.1` to `^1.4.0`, then run `composer update wpboilerplate/wpb-access-control --with-dependencies` — **DONE**: constraint bumped; composer pulled **v1.6.0** (newer than the lockfile's 1.4.0 because patches landed after 2026-06-17). Diff confined to `composer.json` + `composer.lock`. `composer audit`-style line: "No security vulnerability advisories found". T008/T009/T011 were sequenced after T015 and all passed against the new vendor surface.
