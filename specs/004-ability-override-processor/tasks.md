...
- [ ] T010 [US4] Add `AcrossAI_Ability_Override_Processor::bust_cache();` inside the `if ( $deleted )` block in `includes/Modules/Sitewide/Rest/AcrossAI_Sitewide_Override_Controller.php::delete_override()` — W-001 resolution (delete path fires no `acrossai_abilities_sitewide_after_save` hook).
- [ ] T011 [P] [US4] Add `AcrossAI_Ability_Override_Processor::bust_cache();` for the `reset` branch in `includes/Modules/Sitewide/Rest/AcrossAI_Sitewide_Bulk_Controller.php::bulk_action()` — after `delete_override_by_slug()` returns for `reset` action (W-001 resolution — reset path fires no `acrossai_abilities_sitewide_after_save` hook).
...
- [ ] T012 Run PHPCS on all modified files: `includes/Modules/Sitewide/class-acrossai-ability-override-processor.php`, `includes/Modules/Sitewide/Database/AcrossAI_Sitewide_Query.php`, `includes/Main.php`, `includes/Modules/Sitewide/Rest/AcrossAI_Sitewide_Override_Controller.php`, `includes/Modules/Sitewide/Rest/AcrossAI_Sitewide_Bulk_Controller.php`. Fix all errors before proceeding.
...
- [ ] T014 Run PHPUnit `tests/phpunit/sitewide/Test_AcrossAI_Ability_Override_Processor.php` — all 5 test cases must pass (SEC-PLAN-001 GET detection, SEC-PLAN-002 wrapper delegation, SEC-PLAN-003 transient guard).
