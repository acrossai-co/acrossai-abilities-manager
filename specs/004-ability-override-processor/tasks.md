...
- [x] T016 [US1] Enforce `mcp_servers` allowlist in the PATH B block of `boot()` via two real mcp-adapter filters in `includes/Modules/Sitewide/AcrossAI_Ability_Override_Processor.php`:
  - **Filter 1 — DiscoverAbilitiesAbility result**: inside `boot()`, after `add_action( 'wp_abilities_api_init', ... )`, add `add_filter( 'mcp_adapter_tool_call_result', array( __CLASS__, 'filter_discover_abilities_result' ), 10, 5 );`. Implement `public static function filter_discover_abilities_result( $result, array $args, string $tool_name, $mcp_tool, $server ): mixed` — guard `is_array($result) && isset($result['abilities']) && is_array($result['abilities'])`; guard `is_ability_tool($mcp_tool, 'mcp-adapter/discover-abilities')`; get `$server_id` via `$server->get_server_id()`; iterate `$result['abilities']`; for each entry with a string `name`, call `self::is_ability_allowed_on_server($name, $server_id)` and keep only entries that pass.
  - **Filter 2 — ExecuteAbilityAbility pre-call block**: inside `boot()`, after Filter 1, add `add_filter( 'mcp_adapter_pre_tool_call', array( __CLASS__, 'block_execute_ability_by_server' ), 10, 4 );`. Implement `public static function block_execute_ability_by_server( $args, string $tool_name, $mcp_tool, $server ): mixed` — pass through if `is_wp_error($args)`; guard `is_ability_tool($mcp_tool, 'mcp-adapter/execute-ability')`; read `$args['ability_name']`; call `self::is_ability_allowed_on_server($ability_name, $server_id)`; return `new \WP_Error('mcp_server_not_allowed', ...)` to short-circuit execution when not allowed.
  - **Private helpers**: `private static function is_ability_allowed_on_server( string $ability_name, string $server_id ): bool` — calls `wp_get_ability()`, reads `$ability->get_meta()['mcp']['servers']`; `null`/absent → `true` (inherit, all servers); `[]` → `false` (no servers); `[...]` → `in_array($server_id, $allowlist, true)`. `private static function is_ability_tool( $mcp_tool, string $ability_name ): bool` — calls `$mcp_tool->get_adapter_meta()['ability']` (NOT `get_meta()`) to get the original WP ability slug.
  - **Note**: `mcp_adapter_expose_ability` filter does NOT exist in mcp-adapter (confirmed by grep across 34 results / 21 files). All previous references to it were dead code. The filters `mcp_adapter_tool_call_result` (5 args) and `mcp_adapter_pre_tool_call` (4 args) are the real mcp-adapter extension points used here.
  - Run PHPCS and PHPStan level 8 on the modified file after.
...
| T016 — `mcp_adapter_tool_call_result` + `mcp_adapter_pre_tool_call` filters | ✅ Done | — |
