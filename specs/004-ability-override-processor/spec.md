...
4. **Given** a DB override with `mcp_servers = ["server-a"]` exists, **When** the `DiscoverAbilitiesAbility` (`mcp-adapter/discover-abilities`) result is returned to an MCP client, **Then** the ability is present only when the current MCP server ID is `"server-a"` — and absent for all other server IDs. **When** `ExecuteAbilityAbility` (`mcp-adapter/execute-ability`) is called for the ability on a server whose ID is not `"server-a"`, **Then** execution is blocked with a `mcp_server_not_allowed` error. **Note**: `mcp_servers` enforcement operates via `mcp_adapter_tool_call_result` (filter for discovery) and `mcp_adapter_pre_tool_call` (filter for execution blocking) — not via `mcp_adapter_expose_ability` (which does not exist in mcp-adapter).
...
- **FR-009**: `inject_override_args()` MUST write DB override values to the following `$args` paths (null values MUST be skipped per FR-006):
  ...
  | `mcp_servers` | `$args['meta']['mcp']['servers']` | Plugin-specific MCP block; already decoded to `array\|null` by `AcrossAI_Sitewide_Row::__construct()` — guard with `is_array()`, never call `json_decode()`. Runtime enforcement: `filter_discover_abilities_result()` (via `mcp_adapter_tool_call_result`) removes abilities from DiscoverAbilitiesAbility output for non-matching servers; `block_execute_ability_by_server()` (via `mcp_adapter_pre_tool_call`) returns `WP_Error` to block ExecuteAbilityAbility. Tri-state: `null`/absent = all servers; `[]` = no servers; `[...]` = allowlist. |
