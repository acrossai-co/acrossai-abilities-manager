...
- [ ] T004 [SEC-003] [SEC-005] Audit `vendor/acrossai-co/main-menu/` post-install: open the package's `SettingsPage` source and confirm (a) the render path gates on `current_user_can( 'manage_options' )` or stricter, (b) all variable output is wrapped in an `esc_*` function, (c) the form posts to `options.php`. Record the audited commit SHA and the audit outcome at the bottom of `specs/038-acrossai-main-menu-integration/security-review-plan.md` (or in `docs/memory/DECISIONS.md` if capturing as a deviation entry). Files: `specs/038-acrossai-main-menu-integration/security-review-plan.md` (append), `docs/memory/DECISIONS.md` (optional)
