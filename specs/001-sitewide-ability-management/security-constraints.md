...
| Q-04 | What is the maximum length enforced by `sanitize_ability_slug()`? Should match the DB column definition. | LOW-04 / bulk DoS surface | **OPEN — see ST-01 in tasks.md.** Current behavior: silent truncation at 255 via `substr()`. Required: reject > 255 with HTTP 400 via `validate_callback`. |
