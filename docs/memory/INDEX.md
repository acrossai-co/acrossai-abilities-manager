...
| DEC-MCP-TOOLS-PASSTHROUGH-COLUMN | Per-ability MCP tool pass-through column + registry injection (pass_as_tool tinyint, AcrossAI_Ability_Override_Processor::inject_mcp_tools, mcp_adapter_init P20 + Reflection, ARCH-ADV-001) | Abilities/DB | mcp, tools, registry, abilities, berlindb | Active | DECISIONS.md |
...
| BUG-MCP-TYPE-PASSTOOL-CONFLICT | pass_as_tool=1 + mcp_type=resource/prompt → DefaultServerFactory resource error per server; guard inject_mcp_tools() with non_tool_types exclusion | Abilities/MCP | mcp, pass_as_tool, mcp_type, resource, conflict, inject | BUGS.md |
...
| BUG-INJECT-MCP-TOOLS-PERMISSION-BYPASS | inject_mcp_tools() must call check_permissions() as 4th gate — AC rules are fail-open; missing rule silently bypasses manage_options for all users | Abilities/MCP/Security | inject-mcp-tools, permission-bypass, ac-rules, fail-open, pass-as-tool | BUGS.md |
