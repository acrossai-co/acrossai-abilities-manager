...
>
> 4. **Yoast SEO integration (second concrete subclass, 2026-07-27) + kill-switch pattern variant.** Added `includes/Abilities/Integrations/Yoast_SEO.php` as the second concrete `AcrossAI_Integration_Ability_Base` subclass. Yoast has NO single master-switch filter (unlike ACF's `acf/settings/enable_acf_ai`) — each of its 3 abilities is independently gated by a separate analysis feature flag in Yoast's own settings. To preserve the ACF pattern's user-facing behaviour (toggle OFF = abilities gone) without invasively mutating Yoast's own settings, the Yoast subclass uses a **kill-switch variant**: `enable_filter()` is a no-op, and a new `maybe_unregister()` hook wired at `wp_abilities_api_init @ P200` unregisters Yoast's abilities from WP core when the integration toggle is OFF (after Yoast's default-P10 registration). Additional design decisions: (a) `TAB_GROUP = 'yoast'` (NOT `'yoast-seo'`) to avoid colliding with Yoast's own pre-registered category slug (would otherwise let our synthetic rows successfully register into `wp_get_abilities()` with fail-closed callbacks — see `BUG-WP-CORE-ABILITY-CATEGORY-PRE-REGISTRATION`); (b) compound `is_plugin_active()` uses `defined('WPSEO_VERSION') && function_exists('YoastSEO')`; (c) 8 PHPUnit smoke tests in `tests/phpunit/Modules/Library/Integrations/Test_Yoast_SEO.php` cover the constant, slug/label consistency, ability list, no-op `enable_filter`, and the three kill-switch gate branches. Wired at the tail of `Main::define_public_hooks()` alongside the ACF instantiation. Establishes the "kill-switch integration variant" as the second supported model beyond ACF's "enable-filter" model — worth capturing as a memory pattern.
